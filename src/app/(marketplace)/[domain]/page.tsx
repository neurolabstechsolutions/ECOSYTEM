            ...
            const mappedVehicles: Vehicle[] = data.inventory
              ...
              .map((item: any) => {
                ...
                return {
                  ...
                  price: priceUsd,
                  currency: "USD",
                  monthlyEstimate: Math.round(priceUsd / 48),
                  mileage: item.mileage || 0,
                  transmission: (item.transmission || "Automática") as any,
                  fuelType: (item.fuel_type || "Gasolina") as any,
                  bodyType: (item.category_type === "MOTO" ? "Hatchback" : (item.body_type || "SUV")) as any,
                  region: "Barranquilla (Atlántico)",
                  city: item.city || "Barranquilla",
                  exteriorColor: item.exterior_color || "Negro Metalizado",
                  interiorColor: item.interior_color || "Cuero Negro",
                  doors: item.category_type === "MOTO" ? 0 : 5,
                  condition: "Seminuevo Certificado",
                  featured: true,
                  vin: item.vin || item.license_plate || "TRN-98234-CO",
                  plateEnding: item.license_plate ? `Placa: ${item.license_plate}` : "Barranquilla",
                  ...
                  specs: {
                    engine: item.category_type === "MOTO" ? "890 cc CP3 DOHC 4 Válvulas" : "2.8L Turbo Diésel 4x4",
                    horsepower: item.category_type === "MOTO" ? 119 : 204,
                    acceleration: item.category_type === "MOTO" ? "0-100 km/h: 3.2s" : "0-100 km/h: 9.2s",
                    traction: (item.category_type === "MOTO" ? "RWD" : "4x4") as any,
                  },
                  keyFeatures: [
                    "Peritaje 150 Puntos Aprobado",
                    "Garantía Mecánica 1 Año Trinova",
                    "Historial Legal Sin Gravámenes",
                    "Entrega Inmediata"
                  ],
