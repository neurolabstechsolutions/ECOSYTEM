...
export default function TrinovaDedicatedAdminPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 flex flex-col font-sans">
      ...
      <header className="bg-white border-b border-zinc-200/90 sticky top-0 z-30 shadow-xs">
        <div className="max-w-7xl mx-auto px-3 sm:px-5 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono text-zinc-600 font-bold bg-zinc-100 px-1.5 py-0.2 rounded border border-zinc-200">
                  NIT 902.095.222-8
                </span>
              </div>
              <p className="text-[10px] text-zinc-400">Portal Ejecutivo de la Administradora · Barranquilla, Colombia</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-xs">
            <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-semibold ${
              ...
              <span>{connectionStatus === 'CONNECTED' ? `WhatsApp Online (+${connectedNumber || 'Trinova'})` : 'WhatsApp Pendiente'}</span>
            </div>
            ...
            <Link href="/app" className="bg-zinc-900 hover:bg-zinc-800 text-white font-semibold px-2.5 py-1 rounded-lg transition-colors text-xs flex items-center gap-1">
              <span>Torre de Control (NeuroLabs)</span>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-4 md:p-6 space-y-4">
        {/* ─── Compact Clean Metrics Ribbon (Zero Bulky Boxes) ─── */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
          <div className="bg-white border border-zinc-200/90 rounded-xl p-3 shadow-xs space-y-1">
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Línea WhatsApp (Render)</span>
          ...
          <div className="bg-white border border-zinc-200/90 rounded-xl p-3 shadow-xs space-y-1">
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Mandatos de Corretaje</span>
            <div className="flex items-baseline justify-between">
              <span className="text-xl font-black text-zinc-900 font-mono">{MOCK_BROKERAGE.length}</span>
              <span className="text-[11px] font-semibold text-emerald-600">Firmados SHA-256</span>
            </div>
            <p className="text-[10px] text-zinc-400">Validez notarial mercantil</p>
          </div>

          <div className="bg-white border border-zinc-200/90 rounded-xl p-3 shadow-xs space-y-1">
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Portafolio en Consignación</span>
            <div className="flex items-baseline justify-between">
            ...
            <p className="text-[10px] text-zinc-400">Carros, motos e inmuebles</p>
          </div>

          <div className="bg-white border border-zinc-200/90 rounded-xl p-3 shadow-xs space-y-1">
            <span className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Comisiones Proyectadas</span>
            <div className="flex items-baseline justify-between">
        ...
        {/* ─── Compact Nav Switcher (Clean Pills) ─── */}
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 border-b border-zinc-200">
          {[
            { id: 'whatsapp', label: 'Línea WhatsApp & Agente IA (Render)', icon: QrCode },
            { id: 'brokerage', label: `Mandatos de Corretaje (${MOCK_BROKERAGE.length})`, icon: FileText },
            { id: 'sales', label: `Promesas de Compraventa (${MOCK_SALES.length})`, icon: Receipt },
            { id: 'inventory', label: 'Inventario Central (Supabase)', icon: Car },
            { id: 'clients', label: 'Clientes & Compradores', icon: Users },
          ].map(tab => {
            const Icon = tab.icon
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                  isActive ? 'bg-zinc-900 text-white shadow-xs' : 'text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100'
                }`}
              >
